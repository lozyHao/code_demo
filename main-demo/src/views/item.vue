...
<script setup>
import { onMounted, ref, nextTick, render, h } from "vue";
import renders from "@/hook/canvasTrack/render";

let timeLineContainer = ref(null); // canvas外层
let canvas = ref(null); // canvas
let canvasWidth = ref(0); // canvas的宽
let canvasHeight = ref(0); // canvas的高
let ctx = ref(null); // 图画上下文对象

let videoTimes = ref(6000);
let currentTime = ref(0); // 当前时间
let currentZoomIndex = ref(0); // 当前所在时间分辨率的类型索引
const ZOOM = [10, 15, 20, 30, 60, 120, 180, 300, 600]; // 时间分辨率
const ZOOM_HOUR_GRID = [100, 90, 80, 60, 50, 50, 40, 30, 20]; // 每格长度分配
...
const init = () => {
  ...
  gridNum.value = videoTimes.value / ZOOM[currentZoomIndex.value]; //修改绘制格数
  canvasWidth.value = ZOOM_HOUR_GRID[currentZoomIndex.value] * gridNum.value; // canvas宽度
  ...
  timeLineContainerDom.style.width = canvasWidth.value + 10 + "px";
  stageTrankDom.style.width = canvasWidth.value + 10 + "px";
  markTrankDom.style.width = canvasWidth.value + 10 + "px";
  canvas.value.width = canvasWidth.value + 10;
...
const generatingScale = () => {
  for (let i = 0; i < gridNum.value; i++) {
    ...
    } else if (currentZoomIndex.value >= 3 && currentZoomIndex.value < 5) {
...
window.addEventListener("mousemove", (event) => {
  ...
  if (trackingLine.value) {
    trackingLineDom.style.left = trackLeft + event.clientX - 200 + "px";
  } else {
...
// 更新竖轴在视窗中的位置
const updateGrabbingX = () => {
  let rightMarkDom = document.querySelector(".right_mark");
  // 获取轨道视窗宽度,水平移动位置
  const trackWidth = rightMarkDom.clientWidth;
  const trackLeft = rightMarkDom.scrollLeft;
  // 获取竖轴的水平位置
  const grabbingMoveX =
    currentTime.value *
    (ZOOM_HOUR_GRID[currentZoomIndex.value] / ZOOM[currentZoomIndex.value]);
  if (
    grabbingMoveX - trackLeft < 0 ||
    grabbingMoveX - trackLeft > trackWidth - 5
  ) {
    rightMarkDom.scrollLeft = grabbingMoveX - 10;
  }
};
...
const graduationTitle = (item) => {
  const times = item * ZOOM[currentZoomIndex.value];
...
const getTimeHour = (item) => {
  ...
  const minutes = Math.floor((times - hour * 3600) / 60);
  const seconds = Math.floor(times - hour * 3600 - minutes * 60);
